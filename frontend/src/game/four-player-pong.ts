...
export default function initializeFourPlayerPong(): (() => void) | null {
  // Make the function available globally for the HTML file to call
  window.initializeFourPlayerPong = initializeFourPlayerPong;
  console.log("Initializing Four Player Pong");
  ...
  const elements = {
    ...
    scoreboard: document.getElementById("four-player-scoreboard") as HTMLElement | null
  };
  ...
  function startGame(): void {
    console.log("Starting four player game");
  ...
  function openCustomMenu(): void {
    ...
    const customMenu = document.getElementById("four-player-custom-menu");
    if (customMenu) {
      customMenu.classList.remove("hidden");
      customMenu.classList.add("show");
    }

    const pongGame = document.getElementById("four-player-game");
    if (pongGame) {
      pongGame.classList.remove("main-menu");
      pongGame.classList.add("four-player-custom-menu-canvas");
    }
  }
  ...
  function closeCustomMenu(): void {
    ...
    const pongGame = document.getElementById("four-player-game");
    if (pongGame) {
      pongGame.classList.remove("four-player-custom-menu-canvas");
  ...
  function changeColors(event: Event): void {
    ...
    switch (id) {
      case "four-player-ground-color":
        state.color.groundColor = color;
        document.getElementById("fourPlayerCanvas")?.style.setProperty('--ground-color', `${color}`);
        break;
      case "four-player-ball-color":
        state.color.ballColor = color;
        break;
      case "four-player-p1-color":
        state.color.player1Color = color;
        break;
      case "four-player-p2-color":
        state.color.player2Color = color;
        break;
      case "four-player-p3-color":
        state.color.player3Color = color;
        break;
      case "four-player-p4-color":
        state.color.player4Color = color;
        break;
    }
  }
  ...
  function changeWinningScore(event: Event): void {
    ...
    const winningScoreElem = document.getElementById("four-player-winning-score");
    if (winningScoreElem) {
