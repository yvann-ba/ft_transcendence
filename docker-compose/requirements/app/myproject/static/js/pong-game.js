function initializePongGame() {
    ...
    function drawPaddles() {
        ...
        ctx.fillRect(3, state.paddles.player1Y, state.paddles.width, state.paddles.height);
        ctx.fillRect(
            canvas.width - state.paddles.width - 3,
            state.paddles.player2Y,
            state.paddles.width,
            state.paddles.height
        );
    }
    ...
    function checkPaddleCollision() {
        // Joueur 1
        if (state.ball.x - state.ball.radius < state.paddles.width + 3 &&
            state.ball.y > state.paddles.player1Y &&
            state.ball.y < state.paddles.player1Y + state.paddles.height) {
            handlePaddleBounce("player1");
        }
        
        // Joueur 2
        if (state.ball.x + state.ball.radius > canvas.width - state.paddles.width - 3 &&
            state.ball.y > state.paddles.player2Y &&
            state.ball.y < state.paddles.player2Y + state.paddles.height) {
